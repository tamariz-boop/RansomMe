...
#    [ValidateSet( 'WString', 'String', 'Void' )]
    [ValidateSet( 'WString', 'String', 'Void', 'Param' )]
    [String]
    $FuncReturnType = 'Void',
...
# I added this
                'Param' {
                    Write-Verbose "Calling function with Void return type and a string parameter"
                    [IntPtr]$VoidFuncAddr = Get-MemoryProcAddress -PEHandle $PEHandle -FunctionName "VoidFunc"
                    if ($VoidFuncAddr -eq [IntPtr]::Zero)
                    {
                        Throw "Couldn't find function address."
                    }
                    $VoidFuncDelegate = Get-DelegateType @([System.IntPtr]) ([Void])  # char* is represented as a IntPtr
                    $VoidFunc = [System.Runtime.InteropServices.Marshal]::GetDelegateForFunctionPointer($VoidFuncAddr, $VoidFuncDelegate)
                    $FuncStrParam = [System.Runtime.InteropServices.Marshal]::StringToHGlobalAnsi($ExeArgs.Split(' ')[1]) # remove the ReflectiveExe from $ExeArgs
                    $VoidFunc.Invoke($FuncStrParam) | Out-Null
                    [System.Runtime.InteropServices.Marshal]::FreeHGlobal($FuncStrParam)
                }
# end of what I added                
            }
            #########################################
